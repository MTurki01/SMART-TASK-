...
async function saveRemoteState() {
  if (syncingFromRemote || state.mode !== "firebase" || !firebaseDb) return;
  try {
    await set(ref(firebaseDb, "publicTaskflow"), {
      lang: state.lang,
      kind: state.kind,
      source: state.source,
      tasks: state.tasks,
...
function initFirebase() {
  ...
  try {
    ...
    const publicRef = ref(firebaseDb, "publicTaskflow");
...
function boot() {
  const ids = [
    "langToggle","connectionStatus","voiceBtn","taskInput","taskDetails","taskFrequency","taskPriority","taskDue",
    "voicePanel","imagePanel","taskForm","personalSearch","workSearch","personalTasks","workTasks","donePersonal","doneWork",
    "donePersonalCount","doneWorkCount","imageInput",
